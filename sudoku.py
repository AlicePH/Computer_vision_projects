...
def cut(img, contour):
        ...
        tl, tr, br, bl=contour[min(en(1), key=operator.itemgetter(1))[0]][0], contour[max(en(-1), key=operator.itemgetter(1))[0]][0], contour[max(en(1), key=operator.itemgetter(1))[0]][0], contour[min(en(-1), key=operator.itemgetter(1))[0]][0]
...
def table_format(inp_img, scan_tl, scan_br):
    img = inp_img.copy()
    ...
    for x in range(width):
        for y in range(height):
            if img.item(y, x) == 255 and x < width and y < height: img=area
    ...
    return img, np.array([[left, top], [right, bottom]], dtype='float32'), seed_point
def extract_digit(img, rect, size):
    def center(length): return int((size - length) / 2), int((size - length) / 2)+1*(length % 2!=0)
    digit = img[int(rect[0][1]):int(rect[1][1]), int(rect[0][0]):int(rect[1][0])]
    h, w = digit.shape[:2]
    margin = int(np.mean([h, w]) / 2.5)
    bbox= table_format(digit, [margin, margin], [w - margin, h - margin])[1]
    ...
    if w > 0 and h > 0 and (w * h) > 100 and len(digit) > 0: 
      ...
      if h > w:    
          l_pad, r_pad = center(w)    
          digit= cv2.copyMakeBorder(digit, 2, 2, l_pad, r_pad, cv2.BORDER_CONSTANT, None, 0)
      else:
          t_pad, b_pad = center(h)
          digit= cv2.copyMakeBorder(digit, t_pad, b_pad, 2, 2, cv2.BORDER_CONSTANT, None, 0)
      return cv2.resize(digit, (size, size))
    else: 
      return np.zeros((size, size), np.uint8)
def digits_prepare(img, squares, size):
    digits = []
    img_copy=img.copy()
...
def preprocess_table(table):
    ...
    digits = digits_prepare(table, x, 28)
    rows = []
    with_border = [cv2.copyMakeBorder(y.copy(), 1, 1, 1, 1, cv2.BORDER_CONSTANT, None, 255) for y in digits]
    for i in range(9):
...
def digits_answer(proc, clf):
    ...
    for i in range(81):
      ...
      if image.sum() > 10000:
          ...
          sudoku_digits.append(np.argmax(sudoku_digit))
      else:
